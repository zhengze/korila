...
class LdapConn(object):
    def __init__(self, servername, user, password):
        ...
        self.conn = Connection(servername, user=user, password=password, auto_bind=True)
    ...
    def ldap_auth(self, dn, password):
        conn = Connection(self.server, user=dn, password=password, check_names=True, lazy=False, raise_exceptions=False)
        conn.bind()
        return conn.result
